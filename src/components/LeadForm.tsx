...
export default function LeadForm({site, language = 'zh'}: Props) {
  ...
  return <div className={'lead-panel lead-' + site}>
    {status === 'success' ? <div role="status" className="lead-success">
      ...
      <p><a href="mailto:hello@ycomer.xyz">{zh ? '也可以直接写信给 Fankus' : 'Or write directly to Fankus'}</a></p>
      ...
      <p className="lead-fine">{zh ? '无需付费。申请由 Cloudflare 安全保存。' : 'Free to apply. Your request is stored on Cloudflare.'} <a href="mailto:hello@ycomer.xyz">{zh ? '直接联系' : 'Email Fankus'}</a></p>
    </form>}
  </div>;
}
